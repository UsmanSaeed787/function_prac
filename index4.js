...
function tempConverter(tempCel, tempFar) {
    var celsiusToFarenheit = tempCel * (9 / 5) + 32;
    var fahrenheitToCelsius = (tempFar - 32) * (5 / 9);
    console.log(celsiusToFarenheit, fahrenheitToCelsius);
}
tempConverter(37, 98);
//--------------------------------------------------------------------
//  2 - Write a program that calculates the percentage using functions.
function percentageCalculator(totalMarks, obtainedMarks) {
    var numberPercentage = (obtainedMarks / totalMarks) * 100;
...
function daysConverter(numOfDays) {
    let numOfWeeks = Math.floor(numOfDays / 7);
    let days = numOfDays % 7;
    let result = `${numOfWeeks} weeks and ${days} days`;
    return result;
}
...
function discountCalculator(price) {
    if (price > 100) {
        ...
        return `Your discounted price is ${price * 0.95}`;
}
...
function suggestUser(temp1) {
    if (temp1 < 10) {
        ...
        console.log(`The weather is hot outside. Wear light clothes`);
}
suggestUser(30);
//--------------------------------------------------------------------
...
function unitsCheck(bill) {
    if (bill < 200) {
        let totalBill = (bill / 100) * 10;
        console.log(`Your total bill is ${bill + totalBill}`);
    }
    else if (bill < 300) {
        let totalBill = (bill / 100) * 15;
        console.log(`Your total bill is ${bill + totalBill}`);
    }
    else if (bill < 400) {
        let totalBill = (bill / 100) * 20;
        console.log(`Your total bill is ${bill + totalBill}`);
    }
    else {
        let totalBill = (bill / 100) * 25;
        console.log(`Your total bill is ${bill + totalBill}`);
    }
}
unitsCheck(500);
